...
class _NoteReaderScreenState extends State<NoteReaderScreen> {
  ...
  void _italic() {
    ...
    TextSpan newTextSpan = TextSpan(
      text: currentText.substring(selectionStart, selectionEnd),
      style: AppStyle.mainContent.copyWith(fontStyle: FontStyle.italic),
    );

    // Create a new RichText widget with the new TextSpan
    RichText newRichText = RichText(
      text: TextSpan(
        style: AppStyle.mainContent,
        children: [
          TextSpan(
            text: currentText.substring(0, selectionStart),
          ),
          newTextSpan,
          TextSpan(
            text: currentText.substring(selectionEnd),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    var color_id = widget.doc['color_id'];
